...
class AuroraFluSimulation:
    ...
    def _get_vaccination_eligible(self):
        ...
        return eligible
    def perform_anova_analysis(self, intervention_groups=None, metric='attack_rate', num_runs_per_group=5):
        """
        Perform ANOVA analysis to compare intervention groups
        """
        from scipy.stats import f_oneway, levene
        
        # Import scenarios from config if not provided
        if intervention_groups is None:
            try:
                from final_projec.scenarios_config import ANOVA_INTERVENTION_GROUPS
                intervention_groups = ANOVA_INTERVENTION_GROUPS
                print(f"📋 Using scenarios from scenarios_config.py")
            except ImportError:
                print("❌ Could not import scenarios_config. Please provide intervention_groups parameter.")
                return None
        
        print(f"\n🔬 Performing ANOVA Analysis on {metric}")
        print(f"📊 Running {num_runs_per_group} simulations per group...")
        print(f"🎯 Analyzing {len(intervention_groups)} intervention scenarios")
        
        # Store results for each group
        group_data = {}
        all_values = []
        
        # Run simulations for each intervention group
        for group_name, group_params in intervention_groups.items():
            print(f"   🔄 Running {group_name}...")
            
            group_results = []
            
            # Run multiple simulations for this group
            for run in range(num_runs_per_group):
                try:
                    # Create fresh simulation instance for each run
                    sim = self.__class__(**group_params)
                    ...
                    if metric == 'attack_rate':
                        value = result.get('attack_rate', 0)
                    elif metric == 'peak_infected':
                        value = result.get('peak_infected', 0)
                    elif metric == 'total_deaths':
                        value = result.get('total_deaths', 0)
                    elif metric == 'total_infected':
                        value = result.get('total_infected', 0)
                    else:
                    ...
                    print(f"   ⚠️ Simulation {run+1} failed for {group_name}: {e}")
                    # Add a default value to continue analysis
                    group_results.append(0)
                    all_values.append(0)
            
            group_data[group_name] = group_results
            print(f"   ✅ Completed {group_name}: mean {np.mean(group_results):.2f}")
        ...
        group_values = [group_data[group] for group in intervention_groups.keys()]
        
        # Perform one-way ANOVA
        try:
            if len(group_values) >= 2 and all(len(group) >= 2 for group in group_values):
                f_statistic, p_value = f_oneway(*group_values)
                anova_successful = True
                print(f"   📈 ANOVA completed: F={f_statistic:.4f}, p={p_value:.6f}")
            else:
                print("   ⚠️ Insufficient data for ANOVA analysis")
                f_statistic, p_value = 0.0, 1.0
                anova_successful = False
        except Exception as e:
            print(f"   ❌ ANOVA calculation failed: {e}")
            f_statistic, p_value = 0.0, 1.0
            anova_successful = False
        
        # Calculate effect size (eta-squared)
        if anova_successful and len(all_values) > 1:
        ...
        # Test assumptions with Levene's test
        try:
            if len(group_values) >= 2 and all(len(group) >= 2 for group in group_values):
                ...
                levene_p = None
                homogeneity_ok = 'insufficient_data'
        except:
            levene_p = None
            homogeneity_ok = 'test_failed'
        
        # Determine significance
        alpha = 0.05
        significant = p_value < alpha if anova_successful else False
        
        # Simple post-hoc if significant
        post_hoc_summary = ""
        if significant:
            post_hoc_summary = "Pairwise Comparisons:\n"
            ...
            for i, group1 in enumerate(group_names):
                for j, group2 in enumerate(group_names):
                    if i < j:
                        ...
                        post_hoc_summary += f"{group1} vs {group2}: {diff:.2f} difference\n"
        else:
            post_hoc_summary = "Post-hoc analysis not performed - ANOVA was not significant."
        
        print(f"   🎯 Analysis complete: {'Significant' if significant else 'Not significant'} difference found")
        ...
        return {
            ...
            'anova_results': {
                ...
                'degrees_of_freedom_between': len(intervention_groups) - 1,
                'degrees_of_freedom_within': len(all_values) - len(intervention_groups),
                'alpha_level': alpha
            },
            'assumptions': {
                'independence': True,
                'normality': 'assumed',
                'homogeneity_of_variance': homogeneity_ok,
                'levene_test_p_value': levene_p
            },
            'interpretation': {
                'significant_difference': 'Yes' if significant else 'No',
                'effect_size_interpretation': 'large' if eta_squared > 0.14 else 'medium' if eta_squared > 0.06 else 'small' if eta_squared > 0.01 else 'negligible',
                'practical_significance': 'Yes' if eta_squared > 0.01 else 'No'
            },
            'post_hoc': {
                'tukey_hsd_summary': post_hoc_summary,
            ...
            'analysis_info': {
                ...
                'intervention_groups': list(intervention_groups.keys())
            }
        }
